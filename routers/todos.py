...
@router.get("/api/todos/user")
async def api_read_all_by_user(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    if user is None:
        raise get_user_exception()
    return db.query(models.Todos).filter(models.Todos.owner_id == user.get("id")).all()

@router.get("/api/todo/{todo_id}")
async def api_read_todo(todo_id:int,
                    user: dict = Depends(get_current_user),
                    db: Session = Depends(get_db)):
    if user is None:
        raise get_user_exception()
    todo_model = todo_model = db.query(models.Todos)\
...
@router.post("/api/")
async def api_create_todo(todo: Todo,
                      user: dict = Depends(get_current_user),
                      db: Session = Depends(get_db)):
    if user is None:
        raise get_user_exception()
...
async def api_update_todo(todo_id: int, todo: Todo,
                      user: dict = Depends(get_current_user),
                      db: Session = Depends(get_db)):
    if user is None:
        raise get_user_exception()
...
async def read_all_by_user(request: Request, db: Session = Depends(get_db)):
    ...
    todos = db.query(models.Todos).filter(models.Todos.owner_id == user.get("id"))\
        .order_by(desc(models.Todos.id)).all()
